...
namespace EpForceDirectedGraph
{
    public partial class ForceDirectedGraphForm : Form
    {
        ...
        public Pair<int, int> GraphToScreen(Vector3 iPos)
        {
        ...
        public Vector3 ScreenToGraph(Pair<int, int> iScreenPos)
        {
        ...
        public void DrawLine(Edge iEdge, Vector3 iPosition1, Vector3 iPosition2)
        {
            Pair<int, int> pos1 = GraphToScreen(iPosition1);
            Pair<int, int> pos2 = GraphToScreen(iPosition2);
            m_fdgLines[iEdge].Set(pos1.first, pos1.second, pos2.first, pos2.second);
            m_fdgLines[iEdge].DrawLine(paper);
            
        }

        public void DrawBox(Node iNode, Vector3 iPosition)
        {
            Pair<int, int> pos = GraphToScreen(iPosition);
            m_fdgBoxes[iNode].Set(pos.first, pos.second);
            m_fdgBoxes[iNode].DrawBox(paper);
            SolidBrush brush = new SolidBrush(Color.Black);
            paper.DrawString(iNode.Data.label, new Font(FontFamily.GenericSansSerif, 12.0f, FontStyle.Bold), brush, pos.first, pos.second - 22.0f);
        ...
        private void pDrawPanel_MouseMove(object sender, MouseEventArgs e)
        {
            if (clickedNode != null)
            {
                ...
                Vector3 vec = ScreenToGraph(new Pair<int, int>(e.Location.X, e.Location.Y));
